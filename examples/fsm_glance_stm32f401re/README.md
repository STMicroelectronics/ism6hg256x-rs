# ISM6HG256X Sensor Data Acquisition on STM32F401RE with IKS4A1 Shield (JSON-Configured FSM Events)
...
The code is written in Rust using the `stm32f4xx-hal` hardware abstraction layer and the `lsm6hg256x` sensor driver crate. It leverages the **`ucf-tool`** to generate Rust code at build time from JSON (Universal Configuration File) sensor configuration files, enabling flexible sensor FSM configuration.
...
- **Sensor:** ISM6HG256X IMU (accelerometer + gyroscope + FSM-based event detection)
...
The IKS4A1 shield is mounted on top of the STM32F401RE Nucleo board, connecting the ISM6HG256X sensor to the microcontroller's I2C1 peripheral on pins PB8 (SCL) and PB9 (SDA). The sensor's FSM event interrupt line is connected to PC0, configured to trigger an external interrupt on rising edge. UART output is routed through PA2 for serial communication.
...
- The sensor is configured by applying a sequence of register writes and delays generated by the **`ucf-tool`** from a JSON file (`lsm6hg256x_glance_detection.ucf`), which programs the sensor's FSM for gesture detection.
