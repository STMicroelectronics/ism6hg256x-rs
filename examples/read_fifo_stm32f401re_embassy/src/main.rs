            ...
            match f_data.tag {
                Tag::XlNc => {
                    for (index, &data) in raw_array_data.iter().enumerate() {
                        let acc_mg = ism6hg256x_rs::from_fs2_to_mg(data);
                ...
                Tag::XlHg => {
                    for (index, &data) in raw_array_data.iter().enumerate() {
                        let acc_mg = ism6hg256x_rs::from_fs256_to_mg(data);
                ...
                Tag::GyNc => {
                    for (index, &data) in raw_array_data.iter().enumerate() {
                        let angular_rate_mdps = ism6hg256x_rs::from_fs2000_to_mdps(data);
