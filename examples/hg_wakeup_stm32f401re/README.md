# ISM6HG256X Sensor Data Acquisition on STM32F401RE with IKS4A1 Shield (High-G Wakeup Interrupt)
...
- **Sensor:** ISM6HG256X IMU (High-G accelerometer with wakeup detection)
...
The IKS4A1 shield is mounted on top of the STM32F401RE Nucleo board, connecting the ISM6HG256X sensor to the microcontroller's I2C1 peripheral on pins PB8 (SCL) and PB9 (SDA). The sensor's High-G wakeup interrupt line is connected to PC0, which is configured to trigger an external interrupt on rising edge. UART output is routed through PA2 for serial communication.
...
- [lsm6hg256x Rust driver crate](https://crates.io/crates/lsm6hg256x)
