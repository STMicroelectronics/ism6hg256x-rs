# ISM6HG256X Sensor Data Streaming on STM32F401RE Nucleo-64

This example demonstrates continuous data acquisition from the **ISM6HG256X** inertial measurement unit (IMU) sensor on an **STM32F401RE** Nucleo-64 board. It reads accelerometer (low-g and high-g), gyroscope, and temperature data via I2C, averages samples, and outputs the results over UART.
...
- **Sensor:** ISM6HG256X IMU (accelerometer + gyroscope + temperature)
...
The ISM6HG256X sensor is connected via I2C1 on PB8/PB9. The sensor's data ready interrupt line is connected to PC0, configured to trigger an external interrupt on rising edge. UART output is routed through PA2 for serial communication.
...
*This README explains the embedded Rust program for continuous sensor data streaming from the ISM6HG256X on STM32F401RE using Embassy STM32 HAL.*
