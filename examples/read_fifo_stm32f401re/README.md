...
The code is written in Rust using the `stm32f4xx-hal` hardware abstraction layer and the `lsm6hg256x` sensor driver crate.
...
- **Sensor:** ISM6HG256X IMU (accelerometer + gyroscope + timestamp)
...
The IKS4A1 shield is mounted on top of the STM32F401RE Nucleo board, connecting the ISM6HG256X sensor to the microcontroller's I2C1 peripheral on pins PB8 (SCL) and PB9 (SDA). The sensor's FIFO watermark interrupt line is connected to PC0, which is configured to trigger an external interrupt on rising edge. UART output is routed through PA2 for serial communication.
...
- The ISM6HG256X sensor is initialized over I2C.
